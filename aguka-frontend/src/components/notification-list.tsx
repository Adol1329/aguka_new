...
interface Notification {
  ...
  isRead: boolean;
  createdAt: string;
  metadata?: { postId?: string; commentId?: string };
}
...
export function NotificationList() {
  ...
  const handleCardClick = (notification: Notification) => {
    if (notification.metadata?.postId) {
      navigate({ to: "/farmer/community", search: { postId: notification.metadata.postId } });
    }
    ...
    <div className="space-y-4">
      {notifications.map((notification) => (
        <Card
          key={notification.id}
          className={cn(
            notification.isRead ? "opacity-60" : "border-primary",
            notification.metadata?.postId && "cursor-pointer",
          )}
