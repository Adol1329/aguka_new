...
export function RolesManagementComponent() {
  ...
  // Load persisted custom roles on mount
  useEffect(() => {
    superAdminApi
      .getCustomRoles()
      .then((r) => {
        const persisted = r.data || [];
        setCustomRoles(persisted.map((cr: any) => ({ ...cr, isCustom: true, userCount: 0 })));
      })
      .catch(() => {});
  }, []);

  const persistCustomRoles = async (updated: any[]) => {
    const data = updated.map((r: any) => ({
      role: r.role,
      label: r.label,
      description: r.description,
      permissions: r.permissions,
    }));
    await superAdminApi.saveCustomRoles(data);
  };
  ...
  const handleCreateRole = async () => {
    ...
    try {
      await persistCustomRoles(updated);
      toast.success(`Custom role "${newRoleName}" created`);
    } catch {
      toast.error("Failed to persist custom role");
    }
  };
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {role.isCustom && (
                          <Badge
                            variant="outline"
                            className="ml-2 text-[9px] font-black uppercase border-amber-500/30 text-amber-600 bg-amber-500/5"
                          >
                            Custom
                          </Badge>
                        )}
