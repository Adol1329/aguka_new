...
import { Loader2, AlertTriangle, Users, TrendingUp, Activity, MapPin, Coffee } from "lucide-react";
...
function OfficerAnalysisPage() {
  ...
  const {
    ...
    queryFn: () => officerApi.getPerformanceAnalysis().then((r) => (r as any).data),
  });
  ...
  if (analysisError) {
    return (
      <div className="flex flex-col items-center justify-center py-8">
        ...
        <h2 className="text-xl font-bold text-destructive">Failed to load officer analysis</h2>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <PageHeader
          title="Officer Analysis Dashboard"
          subtitle="Overview of your assigned farmers and their performance"
        />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="p-6">
          ...
          <CardContent className="p-0">
            <div className="text-3xl font-bold">
              {analysis.performanceSummary?.avgScore !== null &&
              analysis.performanceSummary?.avgScore !== undefined
                ? `${analysis.performanceSummary.avgScore}%`
                : "0%"}
            </div>
            <p className="text-muted-foreground text-sm">Average across all farmers</p>
          </CardContent>
        </Card>
        ...
        <Card className="p-6">
          ...
          <CardContent className="p-0">
            ...
            <p className="text-muted-foreground text-sm">Total activities recorded</p>
          </CardContent>
        </Card>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex-1 min-w-[100px] flex items-end">
                <Button onClick={handlePerformanceFilter}>Apply Filters</Button>
              </div>
            </div>
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted">
                    <TableHead className="h-12 px-4 text-left text-xs font-bold text-muted-foreground uppercase">
                      Rank
                    </TableHead>
                    <TableHead className="h-12 px-4 text-left text-xs font-bold text-muted-foreground uppercase">
                      Farmer Name
                    ...
                    <TableHead className="h-12 px-4 text-center text-xs font-bold text-muted-foreground uppercase">
                      Soil Moisture (%)
                    </TableHead>
                    <TableHead className="h-12 px-4 text-center text-xs font-bold text-muted-foreground uppercase">
                      Activities
                    ...
                    <TableRow key={farmer.id} className="border-b">
                      ...
                      <TableCell className="px-4 py-4 text-sm">
                        <div className="flex flex-col">
                          ...
                          farmer.cooperativeName !== "Not assigned to cooperative" ? (
                            <div className="text-xs text-muted-foreground">
                              {farmer.cooperativeName}
                            </div>
                          ) : (
                            <div className="text-xs text-muted-foreground">Independent Farmer</div>
                          )}
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-4 text-sm">
                      ...
                      <TableCell className="px-4 py-4 text-sm text-center font-bold">
                        <div
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            farmer.overallScore >= 80
                              ? "bg-success/20 text-success"
                              : farmer.overallScore >= 60
