...
import { Download, Loader2, Trash2, Archive, Save, AlertTriangle } from "lucide-react";
...
function getHumanDescription(log: any): string {
  ...
  if (action === "UPDATE_USER" && before && after) {
    ...
    if (before.role !== after.role) {
      changes.push(
        `Role changed: ${before.role?.toUpperCase() || "NONE"} → ${after.role?.toUpperCase() || "NONE"}`,
      );
    }
...
function AuditPage() {
  ...
  const [archiveBeforeCleanup, setArchiveBeforeCleanup] = React.useState(false);
  const PAGE_SIZE = 20;

  const { data: retentionData, isLoading: loadingRetention } = useAuditRetention();
  const updateRetention = useUpdateAuditRetention();
  const cleanupLogs = useCleanupAuditLogs();

  React.useEffect(() => {
    if (retentionData) {
      setRetentionDays(retentionData.retentionDays ?? 90);
      setArchiveBeforeCleanup(retentionData.archiveBeforeCleanup ?? false);
    }
  }, [retentionData]);
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Button
              variant="outline"
              className={`font-bold ${showRetention ? "border-primary bg-primary/5" : ""}`}
            ...
            <Button
              variant="outline"
              className="border-primary/20 hover:border-primary text-primary hover:bg-primary/5 font-bold"
              onClick={exportCSV}
            >
        ...
        <Card className="p-6 border border-border/50 space-y-5">
            ...
            <Button
              ...
              onClick={() =>
                updateRetention.mutate(
                  { retentionDays, archiveBeforeCleanup },
                  {
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="font-semibold">Retention Period</Label>
              ...
              <p className="text-xs text-muted-foreground">
                Audit logs older than this will be deleted (1–3650 days)
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between rounded-lg border p-4">
                <div>
                  <div className="text-sm font-semibold">Archive Before Cleanup</div>
                  <div className="text-[10px] text-muted-foreground">
                    Export logs to CSV before permanent deletion
                  </div>
                </div>
                <Switch checked={archiveBeforeCleanup} onCheckedChange={setArchiveBeforeCleanup} />
              </div>

              {retentionData && (
                <div className="rounded-lg bg-muted/30 p-4 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Total audit logs</span>
                    <span className="font-bold">
                      {retentionData.totalLogs?.toLocaleString() || 0}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Logs expiring</span>
                    <span className="font-bold text-amber-600">
                      {retentionData.expiringLogs?.toLocaleString() || 0}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="flex justify-end pt-2 border-t border-border/50">
            <Button
              ...
              onClick={() => {
                if (
                  confirm(
                    `Delete all audit logs older than ${retentionDays} days? This cannot be undone.`,
                  )
                ) {
                  cleanupLogs.mutate(undefined, {
                    onSuccess: (r: any) =>
                      toast.success(`Deleted ${r.data?.deletedCount || 0} logs`),
                    onError: (err: any) => toast.error(err.message),
                  });
                }
              }}
            ...
            <span className="text-xs text-muted-foreground">
              ...
              {serverPagination && (
                <>
                  {" "}
                  (server total:{" "}
                  <span className="font-bold text-foreground">{serverPagination.totalItems}</span>)
                </>
              )}
            </span>
          </div>
            ...
            <tbody className="divide-y divide-border/30">
              {paginatedLogs.map((log: any, index: number) => (
                <tr key={log.id} className="group hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider whitespace-nowrap ${actionColor(log.action)}`}
                    >
                      {log.action}
                    </span>
                  </td>
