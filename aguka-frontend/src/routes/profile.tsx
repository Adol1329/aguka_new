  ...
  useEffect(() => {
    if (provinces && formData.province && !formData.provinceCode) {
      const found = provinces.find(
        (p: any) => p.name.toLowerCase() === formData.province.toLowerCase(),
      );
      if (found) {
  ...
  useEffect(() => {
    if (districts && formData.district && !formData.districtCode) {
      const found = districts.find(
        (d: any) => d.name.toLowerCase() === formData.district.toLowerCase(),
      );
      if (found) {
  ...
  useEffect(() => {
    if (sectors && formData.sector && !formData.sectorCode) {
      const found = sectors.find(
        (s: any) => s.name.toLowerCase() === formData.sector.toLowerCase(),
      );
      if (found) {
  ...
  useEffect(() => {
    if (villages && formData.village && !formData.villageCode) {
      const found = villages.find(
        (v: any) => v.name.toLowerCase() === formData.village.toLowerCase(),
      );
      if (found) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-xl border border-border/50 bg-card p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-sm font-black ${pct === 100 ? "text-success" : "text-warning"}`}>
            {pct}%
          </span>
        </div>
        ...
        {missing.length > 0 && (
          <p className="text-[11px] text-muted-foreground mt-2">
            ...
            <span className="font-semibold text-warning">{missing.join(", ")}</span>
          </p>
        )}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="md:col-span-1 p-6 flex flex-col items-center text-center">
          ...
          <div
            className={`mt-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase ${user?.status === "active" ? "bg-success/10 text-success border border-success/20" : "bg-warning/10 text-warning border border-warning/20"}`}
          >
            {user?.status || "Active"}
          </div>
        </Card>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-muted-foreground italic">
                  Phone number is your unique identifier.
                </p>
              </div>
              <div className="space-y-2">
      ...
      {user?.role === "officer" && profile?.officerProfile && (
        <Card className="p-6">
          <h3 className="font-display text-lg font-semibold flex items-center gap-2 mb-5">
            <IdCard className="h-5 w-5 text-primary" /> Officer Credentials
          </h3>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="flex items-center gap-2 text-xs text-muted-foreground">
                <IdCard className="h-3 w-3" /> Employee ID
              </Label>
              <p className="font-mono font-bold text-primary">
                {profile.officerProfile.employeeId || "Not assigned"}
              </p>
            </div>
            <div className="space-y-1.5">
              <Label className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <p className="font-medium">{profile.officerProfile.organization || "—"}</p>
            </div>
            {!!profile.officerProfile.specializations?.length && (
              <div className="space-y-1.5 sm:col-span-2">
                <Label className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Award className="h-3 w-3" /> Specializations
                </Label>
                <div className="flex flex-wrap gap-1.5">
                  {profile.officerProfile.specializations.map((s: string) => (
            ...
            {!!profile.officerProfile.coveredSectors?.length && (
              <div className="space-y-1.5 sm:col-span-2">
                <Label className="flex items-center gap-2 text-xs text-muted-foreground">
                  <MapPin className="h-3 w-3" /> Covered Sectors
                </Label>
                <div className="flex flex-wrap gap-1.5">
                  {profile.officerProfile.coveredSectors.map((s: string) => (
      ...
      {user?.role === "farmer" && profile?.farmerCode && (
        <Card className="p-6">
          <h3 className="font-display text-lg font-semibold flex items-center gap-2 mb-5">
            <IdCard className="h-5 w-5 text-primary" /> Farmer Credentials
          </h3>
          <div className="space-y-1.5">
            <Label className="flex items-center gap-2 text-xs text-muted-foreground">
              <IdCard className="h-3 w-3" /> Farmer ID
            </Label>
            <p className="font-mono font-bold text-primary">{profile.farmerCode}</p>
          </div>
        </Card>
      )}
        ...
        {loadingAudits ? (
          ...
          <div className="space-y-3">
            {auditLogs.map((log: any) => (
              <div
                key={log.id}
                className="flex items-start gap-3 rounded-lg border border-border/30 p-3 hover:bg-muted/20 transition-colors"
              >
      ...
      <Card className="p-6">
        ...
        {loadingAudits ? (
          <div className="flex justify-center py-6">
            ...
            (log: any) => log.action.includes("LOGIN") || log.action.includes("AUTH"),
          ).length > 0 ? (
          <div className="space-y-3">
            {auditLogs
              ...
              .map((l: any, i: number) => (
                <div
                  key={l.id}
                  className="flex items-center justify-between rounded-lg border border-border/30 p-3 hover:bg-muted/20 transition-colors"
                >
                  <div>
