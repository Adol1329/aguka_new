...
export const aiApi = {
  analyzeFarm: async () => {
    return apiClient.post<{ recommendations: AIRecommendation[] }>("/ai/analyze");
  },

  analyzePayload: async (payload: SensorSnapshot) => {
    return apiClient.post<{ recommendations: AIRecommendation[] }>("/ai/recommendations", payload);
  },
  ...
  getHistory: async (params?: { limit?: number; category?: string }) => {
    ...
    return apiClient.get<AIRecommendation[]>("/ai/history", query);
  },

  getFarmRecommendations: async (farmerId: string) => {
    return apiClient.get<AIRecommendation[]>(`/ai/farm/${farmerId}`);
  },

  cooperativeAnalysis: async (cooperativeId?: string) => {
    const query = cooperativeId ? { cooperativeId } : undefined;
    return apiClient.get<any>("/ai/cooperative-analysis", query);
  },
};
