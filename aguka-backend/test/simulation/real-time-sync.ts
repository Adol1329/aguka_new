...
export class RealTimeSync {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('authenticate', async (data) => {
        try {
          ...
          // Verify token and get user info (simplified for demo)
          ...
          if (user) {
            ...
            // Send initial data
            await this.sendInitialData(socket, user.id, farmId);

            logger.info(`Client authenticated: ${socket.id} (${user.role})`);
          } else {
  ...
  /**
   * Authenticate token (simplified for demo)
   */
  private async authenticateToken(token: string): Promise<any> {
    ...
    } else if (token === 'demo_admin_token') {
      return {
        id: 'admin_1',
        role: 'admin',
      };
