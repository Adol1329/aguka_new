...
export class RecommendationController {
        ...
        for (const [type, service] of this.services.entries()) {
          try {
            const recommendation =
              await service.generateRecommendations(farmerId);
            if (recommendation) {
              recommendations.push(recommendation);
            }
          } catch (serviceError) {
            ...
            console.error(
              `Error generating ${type} recommendation:`,
              serviceError,
            );
          }
        }
   ...
   * @param next Express next function
   */
  public acceptRecommendation = async (
    req: RequestWithUser,
    res: Response,
    next: NextFunction,
  ) => {
    try {
      ...
      if (type === "irrigation" && "acceptRecommendation" in service) {
        // @ts-expect-error - We know this method exists for irrigation service
