...
export class RealTimeSync {
  ...
  constructor(
    ...
    irrigationLogic: IrrigationLogic,
  ) {
  ...
  private setupSocketHandlers(): void {
    this.io.on("connection", (socket) => {
        ...
        try {
          ...
          if (decoded) {
            ...
            this.clients.set(socket.id, {
              ...
              userId: decoded.sub,
              role: decoded.role,
              farmId: farmId || (decoded as any).farmId,
            ...
            if (decoded.cooperativeId) {
              socket.join(`cooperative_${decoded.cooperativeId}`);
              socket.join(`community_cooperative_${decoded.cooperativeId}`);
              // Notify others in cooperative about presence
              this.io
                .to(`cooperative_${decoded.cooperativeId}`)
                .emit("user:online", {
                  userId: decoded.sub,
            ...
            // Join district-specific room if profile exists
            try {
              ...
              if (profile?.district) {
                socket.join(`community_district_${profile.district}`);
              }
            } catch (err) {
            ...
            // Join role-based room (super_admin inherits admin broadcasts)
      ...
      socket.on("community:typing", (data) => {
        ...
        if (client && client.cooperativeId) {
          socket.to(`cooperative_${client.cooperativeId}`).emit("user:typing", {
            userId: client.userId,
            userName: client.id, // Fallback or better use name
            postId: data.postId,
      ...
      socket.on("disconnect", () => {
        ...
        if (client) {
          ...
          if (client.cooperativeId) {
            this.io
              .to(`cooperative_${client.cooperativeId}`)
              .emit("user:offline", {
                userId: client.userId,
              });
          }
  ...
  private setupEventListeners(): void {
    ...
    this.sensorEngine.on("sensorData", (data) => {
      // 1. Broadcast as sensor:updated (Documentation standard)
      ...
      // 2. Broadcast as weather:updated (Documentation standard)
      this.broadcastToFarm(data.farmId, {
        type: "weather:updated",
        data: {
          temperature: data.data.airTemperature,
          humidity: data.data.humidity,
          rainfall: data.data.rainfall,
          windSpeed: data.data.windSpeed,
          condition: this.getConditionFromRain(data.data.rainfall),
        },
        timestamp: new Date(),
      });
    ...
    this.irrigationLogic.on("irrigationAction", (data) => {
      const docType: any =
        data.action === "start" ? "irrigation:started" : "irrigation:stopped";
    ...
    // Listen to alerts
    alertService.on("alertTriggered", (data) => {
      this.io.to(`user_${data.userId}`).emit("realtime_update", {
  ...
  private getConditionFromRain(rainfall: number): string {
  ...
  private async getLatestSensorData(farmId: string): Promise<any> {
    const soilService = (await import("../services/soil.service.js"))
      .soilService;
  ...
  private async getIrrigationStatus(farmId: string): Promise<any> {
    const irrigationService = (
      await import("../services/irrigation.service.js")
    ).irrigationService;
  ...
  /**
   * Broadcast community event to a cooperative, district, or global
   */
  broadcastCommunityEvent(
    data: any,
    scope?: { type: string; id?: string },
  ): void {
    const eventName = `community:${data.type}`;
    let targetRoom = "community_global";

    if (scope?.type === "COOPERATIVE" && scope.id) {
      targetRoom = `community_cooperative_${scope.id}`;
    } else if (scope?.type === "DISTRICT" && scope.id) {
      targetRoom = `community_district_${scope.id}`;
    }

    this.io.to(targetRoom).emit(eventName, {
      ...data,
      timestamp: new Date(),
    });

    // Also emit to global for admins if not already global
    if (targetRoom !== "community_global") {
      this.io.to("role_admin").emit(eventName, {
        ...data,
        timestamp: new Date(),
      });
    }
