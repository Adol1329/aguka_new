...
export class PestDiseaseRecommendationModule extends BaseRecommendationService {
  ...
  public async generateRecommendations(
    ...
    try {
      ...
      const shouldGenerate = await this.shouldGenerateNewRecommendation(
        farmerId,
        6,
      ); // 6 hours for pest recommendations
      ...
      if (activeCrops.length === 0) {
        const recommendation: Recommendation = {
          ...
          message:
            "Please set up your farm profile first to receive pest/disease recommendations.",
          recommendation:
        ...
        details: {
          crops: activeCrops.map((crop) => ({
            ...
            growthStage: this.calculateGrowthStage(
              crop.plantedDate,
              crop.crop.growingPeriodDays || 90,
            ),
          })),
          weatherConditions: {
      ...
      const fallbackRecommendation: Recommendation = {
        ...
        recommendation:
          "Unable to generate pest/disease recommendation at this time. Please check your weather data connections.",
        confidence: "low",
        priority: 1,
        actionRequired: false,
        details: {
  ...
  private analyzePestDiseaseRisk(activeCrops: any[], weather: any) {
    ...
    let message =
      "Current conditions present low risk for pest and disease development.";
    let recommendation =
      "Continue regular field monitoring. No immediate action required.";
    let priority = 1;
    ...
    if (humidity > 80 && temp > 20) {
      riskLevel = "high";
      riskFactors.push(
        "High humidity and warm temperatures favor fungal diseases",
      );
      title = "High Fungal Disease Risk";
      message = "Conditions are favorable for fungal disease development.";
      recommendation =
        "Increase field scouting frequency. Consider preventive fungicide application if history of disease.";
      priority = 4;
      actionRequired = true;
      confidence = "high";
    }
    // Moderate humidity and warm temperatures
      ...
      recommendation =
        "Regular field scouting recommended. Monitor for early signs of infestation.";
      priority = 3;
      actionRequired = false;
      confidence = "medium";
    }
    // Dry conditions might increase certain pests
      ...
      riskLevel = "medium";
      riskFactors.push(
        "Dry and warm conditions may favor certain insect pests",
      );
      title = "Potential Insect Pest Risk";
      message = "Dry conditions may increase activity of certain insect pests.";
      recommendation =
        "Monitor for insect pests, especially in dry areas of the field.";
      priority = 2;
      actionRequired = false;
      confidence = "medium";
    }
    ...
    if (vulnerableCrops.length > 0 && riskLevel !== "low") {
      riskFactors.push(
        `Vulnerable crops detected: ${vulnerableCrops.map((c) => c.crop.nameEn).join(", ")}`,
      );
  ...
  private calculateGrowthStage(
    plantedDate: Date,
    totalGrowingPeriodDays: number,
  ): string {
    const daysSincePlanting = Math.floor(
      (Date.now() - plantedDate.getTime()) / (1000 * 60 * 60 * 24),
    );
    const progressPercentage =
      (daysSincePlanting / totalGrowingPeriodDays) * 100;
...
export const pestDiseaseRecommendationModule =
  new PestDiseaseRecommendationModule();
