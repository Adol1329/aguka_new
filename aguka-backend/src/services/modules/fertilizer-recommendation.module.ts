...
export class FertilizerRecommendationModule extends BaseRecommendationService {
  ...
  public async generateRecommendations(
    ...
    try {
      ...
      const shouldGenerate = await this.shouldGenerateNewRecommendation(
        farmerId,
        12,
      ); // 12 hours for fertilizer recommendations
      ...
      if (activeCrops.length === 0) {
        const recommendation: Recommendation = {
          ...
          message:
            "Please set up your farm profile first to receive fertilizer recommendations.",
          recommendation:
            "No active crops found. Please set up your farm profile first.",
          confidence: "low",
          priority: 1,
      ...
      const fertilizerAnalysis = this.analyzeFertilizerNeeds(
        activeCrops,
        soilStatus,
      );
      ...
      const fallbackRecommendation: Recommendation = {
        ...
        recommendation:
          "Unable to generate fertilizer recommendation at this time. Please check your soil data connections.",
        confidence: "low",
        priority: 1,
        actionRequired: false,
        details: {
  ...
  private analyzeFertilizerNeeds(activeCrops: any[], soilStatus: any) {
    ...
    let recommendation =
      "No fertilizer application needed at this time based on current soil tests.";
    ...
    if (totalDeficit > 30) {
      // Significant deficiency threshold
      priority = 4;
      actionRequired = true;
      confidence = "high";
      title = "Fertilizer Application Recommended";
      message =
        "Soil tests indicate nutrient deficiencies that may affect crop performance.";
      recommendation =
        "Consider applying fertilizer to address nutrient deficiencies.";

      if (nDeficit > 15) {
        specificRecommendations.push(
          `Apply nitrogen fertilizer at ${Math.round(nDeficit)} kg/ha`,
        );
      }
      if (pDeficit > 10) {
        specificRecommendations.push(
          `Apply phosphorus fertilizer at ${Math.round(pDeficit)} kg/ha`,
        );
      }
      if (kDeficit > 10) {
        specificRecommendations.push(
          `Apply potassium fertilizer at ${Math.round(kDeficit)} kg/ha`,
        );
      }
      ...
      message =
        "Soil tests show minor nutrient deficiencies. Monitor crops for signs of deficiency.";
      recommendation =
        "Consider soil retesting in 4-6 weeks. No immediate fertilizer application needed.";
      ...
      if (pDeficit > 3) {
        specificRecommendations.push(
          "Monitor for phosphorus deficiency symptoms",
        );
      }
      if (kDeficit > 3) {
        specificRecommendations.push(
...
export const fertilizerRecommendationModule =
  new FertilizerRecommendationModule();
